...
private val BASE_EN: Map<String, String> = mapOf(
    ...
    "games.badge.hdInstalled" to "HD texture pack installed. Opens Texture Packs.",
    "games.badge.hdAvailable" to "HD texture pack available to download. Opens Texture Packs.",
    "games.badge.hdInstalled.short" to "Installed",
    "games.badge.discs" to "%d DISCS",
    "games.menu.playDisc" to "Play Disc %d",
    "action.insertDisc" to "Insert Disc %d",
    "games.badge.hdAvailable.short" to "Available",
    ...
    "renderer.textureUpscale.title" to "Texture Upscaling",
    "renderer.textureUpscale.about" to "Upscales each texture as the emulator uploads it, not the finished screen. The result is cached, so a texture is only ever scaled once. Independent of the internal resolution and of texture packs.",
    "renderer.textureUpscale.world.label" to "Upscale world textures",
    "renderer.textureUpscale.world.description" to "3D surfaces and anything larger than 128x128.",
    "renderer.textureUpscale.ui.label" to "Upscale UI textures",
    "renderer.textureUpscale.ui.description" to "HUD, fonts and 2D art, 128x128 and smaller. Crisp text often looks worse upscaled — try it before leaving it on.",
    "renderer.textureUpscale.algorithm.label" to "Algorithm",
    "app.devServer" to "Dev server (MCP)",
    "app.devServer.desc" to "Local control server for development: boot games, change settings, capture screenshots and read upscaler stats from a PC over adb. Listens on 127.0.0.1 only; never on the network. Shows an MCP chip while running.",
    "renderer.textureUpscale.family.learned" to "Learned",
    "renderer.textureUpscale.algorithm.raisrhd" to "RAISR-HD - kernels learned from HD texture packs, so a game with no pack gets a sharper, edge-aware upscale on the fly. Not a neural net: a few hundred operations per pixel on the worker thread. Sharpens along edges without inventing detail; pixel-art sprites may still prefer xBR.",
    "renderer.textureUpscale.reload.label" to "Reload textures",
    "renderer.textureUpscale.reload.description" to "Re-runs every visible texture through the current filter (and any pack you just installed). Otherwise a change only reaches textures loaded from now on.",
    "renderer.textureUpscale.reload.toast" to "Reloading textures...",
    "renderer.textureUpscale.reload.noGame" to "No game running",
    "renderer.textureUpscale.family.resample" to "Resample",
    "renderer.textureUpscale.family.pixelart" to "Pixel art",
    "renderer.textureUpscale.family.edge" to "Edge-directed",
    "renderer.textureUpscale.family.neural" to "Neural",
    "renderer.textureUpscale.family.neuralHint" to "Needs a trained model in <textures>/models. None ship with ARMSX2, so these leave textures untouched until you install one.",
    "renderer.textureUpscale.scale.label" to "Scale",
    "renderer.textureUpscale.algorithm.nearest" to "Nearest — no filtering at all. Useful as the baseline to judge the others against.",
    "renderer.textureUpscale.algorithm.bilinear" to "Bilinear — soft and safe, the do-no-harm option.",
    "renderer.textureUpscale.algorithm.sharpbilinear" to "Sharp Bilinear — crisp texel edges with a one-pixel ramp. Looks unfiltered, without nearest's uneven blocks.",
    "renderer.textureUpscale.algorithm.mitchell" to "Mitchell — softer than Bicubic but never rings. The safer choice on hard colour steps.",
    "renderer.textureUpscale.algorithm.bicubic" to "Bicubic — sharper than bilinear without ringing. Good all-rounder.",
    "renderer.textureUpscale.algorithm.lanczos" to "Lanczos — sharpest plain resample. Can ring on hard edges.",
    "renderer.textureUpscale.algorithm.lanczoscas" to "Lanczos + CAS — Lanczos, then contrast-adaptive sharpening. Best neutral pick.",
    "renderer.textureUpscale.algorithm.scale2x" to "Scale2x — sharp edges, never invents a colour. Best for pixel art.",
    "renderer.textureUpscale.algorithm.eagle" to "Eagle — chunkier than Scale2x, a more nostalgic look.",
    "renderer.textureUpscale.algorithm.supereagle" to "SuperEagle — Eagle with smoother diagonals. Softer than Scale2x.",
